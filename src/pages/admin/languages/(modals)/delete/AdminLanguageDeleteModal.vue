...
import { useLanguageDelete } from "@/api/languages/delete/useLanguageDelete";
...
const handleDelete = async () => {
  try {
    await deleteLanguage.mutateAsync({ languageId: props.id });
    toast.success("Language deleted successfully");

    isOpen.value = false;
    emit("success");
  } catch (e) {
    toast.error("Error while deleting language");
  }
};
</script>
  ...
  <Dialog v-model:open="isOpen">
    ...
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Here you can delete the language.
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter>
        ...
        >
        <Button
          :disabled="deleteLanguage.isLoading.value"
          variant="destructive"
          @click="handleDelete"
          class="cursor-pointer"
        >
          {{ deleteLanguage.isLoading.value ? "Deleting..." : "Delete" }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
